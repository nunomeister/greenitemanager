export type ServiceCode = 'meisters_pick' | 'greenite_detected' | 'infection_alert' | 'greenite_spread' | 'mutation_detected';
...
export const SERVICE_META: Record<ServiceCode, { name: string; emoji: string; colorClass: string; badgeClass: string; ringClass: string }> = {
  ...
  greenite_spread:    { name: 'Greenite Spread',    emoji: '🟢', colorClass: 'text-success',         badgeClass: 'bg-success/15 text-success border-success/30',    ringClass: 'ring-success/40' },
  mutation_detected:  { name: 'Mutation Detected',  emoji: '🔴', colorClass: 'text-destructive',     badgeClass: 'bg-destructive/15 text-destructive border-destructive/30', ringClass: 'ring-destructive/40' },
};
